...
import { Mail, Lock, Eye, EyeOff, Facebook, Phone } from 'lucide-react';
...
export function SignInPage() {
  ...
  const [loginMethod, setLoginMethod] = useState<'email' | 'facebook' | 'phone' | 'tiktok'>('email');
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Sign in to your GameTopUp account
            </p>
          </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                <label className="block text-sm text-gray-700 dark:text-gray-300 mb-2">
                  ...
                  {loginMethod === 'tiktok' && 'TikTok Username'}
                </label>
                {loginMethod === 'phone' ? (
                  ...
                  <div className="relative">
                    <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                      ...
                      {loginMethod === 'facebook' && <Facebook className="w-5 h-5 text-gray-400" />}
                      {loginMethod === 'tiktok' && <svg className="w-5 h-5 text-gray-400" viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z" /></svg>}
                    </div>
                    <input
                      type={loginMethod === 'tiktok' ? 'text' : 'email'}
                      value={email}
                      onChange={(e) => setEmail(e.target.value)}
                      placeholder={
                        loginMethod === 'email' ? 'your.email@example.com' :
                        loginMethod === 'facebook' ? 'facebook@example.com' :
                        '@username'
                      }
                      required
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  type="button"
                  onClick={() => { setLoginMethod('facebook'); setEmail(''); }}
                  className={`flex items-center justify-center gap-2 px-4 py-3 border-2 rounded-lg transition-colors ${
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  type="button"
                  onClick={() => { setLoginMethod('tiktok'); setEmail(''); }}
                  className={`flex items-center justify-center gap-2 px-4 py-3 border-2 rounded-lg transition-colors ${
                    loginMethod === 'tiktok' 
                      ? 'border-purple-600 bg-purple-50 dark:bg-purple-900/30' 
                      : 'border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700'
